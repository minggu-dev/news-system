...
초기 사이트 진입 시 바로 기사 목록을 보여주기보다 먼저 관심 카테고리를 선택하도록 구성했습니다. 이를 통해 사용자는 원하는 분야의 기사만 빠르게 진입할 수 있으며, `전체` 탭을 함께 제공하여 모든 기사를 발행 시간 순으로 확인할 수 있습니다.
...
카테고리 선택 후 기사 리스트 화면으로 진입하면 읽은 기사와 읽지 않은 기사를 시각적으로 구분합니다. 읽지 않은 기사는 강조된 제목과 `읽지 않음` 배지로 표시하고, 읽은 기사는 낮은 투명도와 `읽음` 상태로 표시하여 사용자가 이미 확인한 기사를 자연스럽게 구분할 수 있도록 했습니다.
